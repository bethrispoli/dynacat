function [session, init_params, glm_params] = dynacat_AnalysisParams(session, clip)
% 
...
init_params = dynacat_initDefaultParams;
...
niifiles = dir(fullfile(session, 'functionals', '*.nii.gz')); niifiles = {niifiles.name};
niifiles = niifiles(~contains(niifiles,'._'));
parfiles = dir(fullfile(session, 'parfiles', '*.par')); parfiles = {parfiles.name};
parfiles = parfiles(~contains(parfiles,'._'));
...
while sum(contains(lower(niifiles), ['run' num2str(num_runs + 1) '.nii.gz'])) >= 1
    ...
    niipaths{num_runs} = fullfile(session, 'functionals', niifiles{nii_idx});
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    par_idx = find(contains(lower(parfiles), ['run' num2str(num_runs)]), 1);
    if isempty(par_idx)
        fprintf('Error: no .par file found for run %d. \n\n', num_runs); return;
    else
       
        %parpaths{num_runs} = fullfile(session, parfiles{par_idx});
        parpaths{num_runs} = fullfile(session, 'parfiles', parfiles{par_idx});
...
for i = 1:length(niipaths)
    annotations{i} = ['dynacat_run',num2str(i)];
end
...
%     nii = niftiRead(fullfile(niipaths{1})); nii.data = mean(nii.data, 4);
...
%     inplane = fullfile(fullfile('raw',niifiles{inplane_idx}));
% end
% % inplane = params.inplane; %can't we just call the inlpane path that we found in dynacat_initDefaultParams.m?

InplanePath = fullfile(session, 'Inplane');
inplane_file_name = dir(fullfile(InplanePath, '*.nii.gz'));
inplane = fullfile(InplanePath, inplane_file_name.name);


% get the durations of TR and events
nii = niftiRead(fullfile(niipaths{1})); TR = nii.pixdim(4);
pid = fopen(fullfile(parpaths{1})); 
...
%% Set preprocessing parameters -- whats the point of making my default params if this just resets them all??
...
init_params.sliceTimingCorrection = 0;  % logical, do/don't do slice time correction
init_params.sliceOrder = [1:2:27,2:2:28]; % Alternating increasing starting at first slice, ending at last
...
init_params.doSkipFrames      = 0; % logical, clip countdown frames during initialization
...
init_params.applyCorAnal      = 0; % logical, don't apply CorAnal during 
                                   %          init
init_params.motionComp        = 1; % logical, don't do motion compensation
                                   %          during init, we'll do it
                                   %          later
...
init_params.description = 'dynacat'; % char array describing session
init_params.comments    = 'Analyzed using with beths new dynacat code'; % char array of comments
...
% GLM annotation
glm_params.annotation = sprintf('dynacatGLM_%iruns',length(init_params.functionals));
