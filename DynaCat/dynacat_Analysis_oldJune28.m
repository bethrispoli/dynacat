...
%
% testing with dynacat_Analysis('/home/brispoli/DynamicCategories/data/DynaCat/subj-02/session1/')


%% Set subject session path
% ex. '/home/brispoli/DynamicCategories/data/DynaCat/subj-02/session1/'

session_path = '/home/brispoli/DynamicCategories/data/DynaCat/subj-02/session1/';
...
if year < 2016
    fprintf('Error: dynacatAnalysis requires MATLAB version 2016 or later');
    return;
end

% check session argument and get session_id
if nargin < 1 || isempty(session_path) || ~(exist(session_path, 'dir') == 7)
    fprintf('Error: path to "subject" data directory not found. \n\n'); return;
else
    [~, session_id] = fileparts(session_path);
end
...
% for any number of conditions, contrast each individual condition with all others
for cc = 1:length(cond_num_list)
